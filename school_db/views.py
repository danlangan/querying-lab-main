...
def problem_one(request):
  ...
  student_3_gpa = Student.objects.filter(gpa__gt=3).order_by('-gpa')

  for student in student_3_gpa:
...
def problem_two(request):
  instructors_before_2010 = Instructor.objects.filter(hire_date__year__lt=2010).order_by('hire_date__year')
...
def problem_three(request):
  ...
  instructor_pk_2 = Instructor.objects.filter(id=2).get()
  instructor_pk_2_classes = Course.objects.filter(instructor_id=2)
  print(f'''
  Instructor Name: {instructor_pk_2.first_name} {instructor_pk_2.last_name}
  Courses: ''')

  for class_name in instructor_pk_2_classes:
...
def problem_six(request):
    ...
    new_student_gpa_update = Student.objects.filter(id=11).update(gpa__=3.75)
    new_student_gpa_update = Student.object.filter(id=11).get()
    print(f'''
    Id: {new_student_gpa_update.id}
    Full Name: {new_student_gpa_update.first_name} {new_student_gpa_update.last_name}
    GPA: {new_student_gpa_update.gpa}
    ''')
    return complete(request)
